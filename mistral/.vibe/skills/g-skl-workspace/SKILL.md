...
6. **Workspace-role choice (T1454)**: if `--role` was NOT supplied, run the [Member Role Choice Prompt](#member-role-choice-prompt-t1454) before reporting the planned manifest fields. Do not silently default to `controlled_member`. When the role is supplied via `--role`, skip the prompt.
7. Report the manifest fields that would be added, including `allowed_write_policy.default_policy` and `workspace_role`.
8. Default controlled members to `write_allowed: false` unless the active task explicitly authorizes a write-enabled member.
...
- `--role autonomous_child|controlled_member`: workspace role for the new member. When omitted, run the [Member Role Choice Prompt](#member-role-choice-prompt-t1454) (default recommended `autonomous_child`); do not silently assume `controlled_member`.
...
6. **Workspace-role choice (T1454)**: if `--role` was NOT supplied, run the [Member Role Choice Prompt](#member-role-choice-prompt-t1454) before reporting the planned manifest entry. The default recommended answer is `autonomous_child`. When `autonomous_child` is chosen, the new member receives the full gald3r framework via the T1452 installer after the git root + manifest entry are created (see the prompt section); when `controlled_member` is chosen, only the marker-only `.gald3r/` is bootstrapped.
7. If the path is absent, report the exact folder that would be created.
8. Report the manifest entry that would be added (`workspace_role` reflects the chosen role):
...
7. **Role-dependent `.gald3r/` materialization (T1454 / g-rl-36)**:
...
- Do not install gald3r into the member repo (the full installer runs only for `autonomous_child` targets per step 7).
...
After apply, the g-rl-36 guard allows `@g-skl-setup` on the repo. The promoted repo is now an
`autonomous_child` and MUST receive the **complete** gald3r framework -- not just the `.gald3r/`
files PROMOTE scaffolds. Run the full installer on the promoted path (T1452):
