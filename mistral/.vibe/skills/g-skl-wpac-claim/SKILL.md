...
### 6. Write peer copy
Write `peers/<parent_project_name>.md` in current project's `workspace/peers/`:
```markdown
...
a) Create `<parent_path>/.gald3r/workspace/` if missing (+ INBOX.md, README.md, peers/)

b) Read or initialize `<parent_path>/.gald3r/workspace/topology.md`
...
g) Write `<parent_path>/.gald3r/workspace/peers/<current_project_name>.md`:
```markdown
...
CLAIMED ✓
  ...
  Updated : <parent_project_path>/.gald3r/workspace/topology.md  [or "skipped (--one-way)"]
