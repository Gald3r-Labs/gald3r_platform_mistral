...
Difference from `g-skl-wpac-move`:
- `g-skl-wpac-move` requires the destination to already be in topology and enforces strict pre-flight
- `g-skl-wpac-send-to` is more flexible: works with freshly spawned projects, supports partial sends,
...
Next: open <dest_project> and run @g-wpac-read to review the incoming content.
```
...
|------------------------|--------------------------|
| Transferring content to an existing or newly spawned project | `g-skl-wpac-move` for strict topology-gated migrations with full pre-flight |
| Forwarding features/ideas/bugs | `g-skl-wpac-order` for pushing tasks (not content) |
| Seeding a new project after spawn | `g-skl-wpac-spawn --features` handles this at spawn time |
| Notifying without sending files | `g-skl-wpac-notify` for FYI-only messages |
