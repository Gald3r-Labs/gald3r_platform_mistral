...
<h1 align="center">Gald3r for Mistral</h1>

<p align="center">
  The <strong>Mistral edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside Mistral</strong>. Part of the Gald3r framework
...
This is the **Mistral distribution** of Gald3r. The [`mistral/`](./mistral/) folder is a
complete, ready-to-deploy gald3r setup tuned for Mistral -- drop its contents into your
...
cp -r gald3r_platform_mistral/mistral/. /path/to/your/project/
```

Then open the project in Mistral and type `@g-status` to confirm gald3r loaded.

> The `mistral/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
